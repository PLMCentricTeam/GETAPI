...
let inactivityTime = 0; // Menampung waktu tidak aktif dalam detik

// Event listener untuk event apapun pada halaman
addEventListener('click', resetInactivityTimer);
addEventListener('keydown', resetInactivityTimer);
addEventListener('mousemove', resetInactivityTimer);

// Fungsi untuk mereset waktu tidak aktif
function resetInactivityTimer() {
    inactivityTime = 0; // Reset waktu tidak aktif menjadi 0
    startInactivityTimer(); // Mulai timer baru
}

// Fungsi untuk memulai timer tidak aktif
function startInactivityTimer() {
    setInterval(function() {
        inactivityTime++; // Tambah waktu tidak aktif setiap detik
        if (inactivityTime >= 600) { // Periksa apakah 10 menit telah berlalu (600 detik)
            logout(); // Jalankan fungsi logout
        }
    }, 1000); // Jalankan fungsi setiap detik (1000 milidetik)
}
...
function logout() {
    ...
    window.location.href = '../index.html'; // Sesuaikan path ke halaman login
}

// Panggil fungsi startInactivityTimer saat halaman dimuat
startInactivityTimer();

function fetchData() {
    // Mengimpor token.js
    const script = document.createElement('script');
    script.src = 'SecurityToken.js';
    document.head.appendChild(script);
    script.onload = function() {
        // Memastikan script telah dimuat
        if (typeof securityToken !== 'undefined') {
            const token = securityToken;
            console.log("Token:", token);
            // Anda dapat menggunakan token di sini untuk kebutuhan lebih lanjut
            ...
            var xhr = new XMLHttpRequest();
                ...
                function displayData(jsonData) {
                    ...
                    jsonData.forEach(item => {
                        ...
                        //Function to define Cell Season
                        function displayDataSeason() {
                            const cellSeason = row.insertCell(10);
                            cellSeason.textContent = item.bag_season;
                            //Function Validation Rule for Sizes
                            function isSeasonInMasterData(item) {
                                for (let i = 0; i < MasterSeason.length; i++) {
                                    if (MasterSeason[i] === item.bag_season) {
                                        return true;
                                    }
                                }
                                return false;
                            ...
                            //Validation Sizes    
                            if (!isSeasonInMasterData(item)) {
                                cellSeason.classList.add('invalid');
                            }
                        }
                            ...
                            ColorwayList.forEach(item => { 
                                ...
                                if(this.readyState === 4) {
                                    ...
                                    if (jsonDataColorway.color_specification !== 'centric%3A' && !isColorwayInMasterData(jsonDataColorway)) {
                                        const ListColorway = document.createElement("tr"); // Create new data based on Active Colorway
                                        ListColorway.id = "TableData";
            ...
            console.error("Security token is not defined.");
        }
    };
}
